...
def save_ticker(entry: dict):
    conn = get_conn()
    # INSERT OR IGNORE preserves added_price (and history) if the ticker already exists.
    # A separate UPDATE then refreshes only the editable metadata fields.
    conn.execute("""
        INSERT OR IGNORE INTO ticker_tracker
    ...
    if conn.execute("SELECT changes()").fetchone()[0] == 0:
        # Row already existed — update metadata but never touch added_price or added_at
        conn.execute("""
            UPDATE ticker_tracker
               SET name = ?, note = ?, pmid = ?, nct_id = ?
             WHERE ticker = ?
        """, (
            entry.get("name"), entry.get("note"), entry.get("pmid"),
            entry.get("nct_id"), entry["ticker"],
        ))
    conn.commit()
    conn.close()
...
def update_price(ticker: str, data: dict):
    ...
    # Sort by date so the chart always renders in chronological order
